...
pub fn run_da_signers_call<CTX: ContextTr>(
    ...
    // revm-41 migration: PrecompileOutput::new now requires reservoir; the original calls passed only gas_used and bytes.
    match selector {
        DASigners::paramsCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_params_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::epochNumberCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_epoch_number_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::quorumCountCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_quorum_count_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::getSignerCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_get_signer_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::getQuorumCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_get_quorum_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::getQuorumRowCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_get_quorum_row_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::registerSignerCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_register_signer_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::updateSocketCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_update_socket_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::registerNextEpochCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_register_next_epoch_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::getAggPkG1Call::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_get_agg_pk_g1_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::isSignerCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_is_signer_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::registeredEpochCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_registered_epoch_call(input_bytes, caller, value, context)?,
            0,
        )),
        DASigners::makeEpochCall::SELECTOR => Ok(PrecompileOutput::new(
            gas_used,
            run_make_epoch_call(input_bytes, caller, value, context)?,
            0,
        )),
...
fn run_epoch_number_call<CTX: ContextTr>(
    ...
    Ok(Bytes::from(epochNumberCall::abi_encode_returns(
...
fn run_quorum_count_call<CTX: ContextTr>(
    ...
    let args = quorumCountCall::abi_decode_validate(input_bytes)
        .map_err(|_e| PrecompileError::StatefulInvalidInput)?;
    Ok(Bytes::from(quorumCountCall::abi_encode_returns(
        &quorum_count(
            context,
            args._epoch
                .try_into()
...
fn run_get_quorum_call<CTX: ContextTr>(
    ...
    let epoch: u64 = args
        ...
        .try_into()
        .map_err(|_e| PrecompileError::StatefulInvalidInput)?;
    let quorum_id: u64 = args
        ._quorumId
        .try_into()
        .map_err(|_e| PrecompileError::StatefulInvalidInput)?;
    Ok(Bytes::from(getQuorumCall::abi_encode_returns(&get_quorum(
        context, epoch, quorum_id,
    )?)))
}
...
fn run_get_quorum_row_call<CTX: ContextTr>(
    ...
    Ok(Bytes::from(getQuorumRowCall::abi_encode_returns(
        &quorum[args._rowIndex as usize],
    )))
}
...
fn run_register_next_epoch_call<CTX: ContextTr>(
    ...
    if let Some(signer) = get_signer(context, args.signer)? {
        ...
        Ok(Bytes::from(registerNextEpochCall::abi_encode_returns(
            &registerNextEpochReturn {},
        )))
    } else {
        Err(PrecompileError::Other("signer not found".to_string()))
    }
...
fn run_registered_epoch_call<CTX: ContextTr>(
    ...
    Ok(Bytes::from(registeredEpochCall::abi_encode_returns(
        &registered,
    )))
}
...
fn epoch_block<CTX: ContextTr>(context: &mut CTX, epoch: U256) -> Result<U256, PrecompileError> {
    Ok(context
        ...
        .sload(
            DA_SIGNERS_ADDRESS,
            epoch_block_key(epoch.try_into().unwrap()).into(),
        )
        .map_err(convert_db_err::<CTX::Db>)?
        .data)
}
...
fn epoch_registered_signer<CTX: ContextTr>(
    ...
    let b: &[u8; 32] = &context
        ...
        .sload(
            DA_SIGNERS_ADDRESS,
            epoch_registered_signer_key(epoch, index).into(),
        )
        .map_err(convert_db_err::<CTX::Db>)?
        .data
        .to_be_bytes();
...
fn run_make_epoch_call<CTX: ContextTr>(
    ...
    if last_epoch_block > U256::from(0) && block_height < last_epoch_block + p.epochBlocks {
        ...
        return Ok(Bytes::from(makeEpochCall::abi_encode_returns(
            &makeEpochReturn {},
        )));
    }
    // new epoch
    ...
    for (index, quorum) in quorums.iter().enumerate() {
        ...
        store_bytes(
            context,
            DA_SIGNERS_ADDRESS,
            quorum_key(epoch_u64, index as u64),
            &buf,
        )?;
    }
    context
        ...
        .sstore(
            DA_SIGNERS_ADDRESS,
            quorum_count_key(epoch_u64).into(),
            U256::from(quorums.len()),
        )
        .map_err(convert_db_err::<CTX::Db>)?;
    ...
    context
        ...
        .sstore(
            DA_SIGNERS_ADDRESS,
            epoch_block_key(epoch_u64).into(),
            block_height,
        )
        .map_err(convert_db_err::<CTX::Db>)?;

    Ok(Bytes::from(makeEpochCall::abi_encode_returns(
        &makeEpochReturn {},
    )))
}
...
fn run_get_agg_pk_g1_call<CTX: ContextTr>(
    ...
    Ok(Bytes::from(getAggPkG1Call::abi_encode_returns(
        &getAggPkG1Return {
