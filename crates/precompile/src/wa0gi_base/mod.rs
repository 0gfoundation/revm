...
fn selectors_map() -> &'static HashMap<[u8; 4], (u64, bool)> {
    SELECTORS.get_or_init(|| {
        ...
        m.insert(WrappedA0GIBase::setMinterCapCall::SELECTOR, (100_000, false));
...
pub fn run_wa0gi_base_call<CTX: ContextTr>(
    ...
    match selector {
        WrappedA0GIBase::mintCall::SELECTOR => {
            Ok(stateful_output(gas_used, run_mint_call(input_bytes, caller, value, context)?))
        }
        WrappedA0GIBase::burnCall::SELECTOR => {
            Ok(stateful_output(gas_used, run_burn_call(input_bytes, caller, value, context)?))
        }
        WrappedA0GIBase::setMinterCapCall::SELECTOR => Ok(stateful_output(
            gas_used,
            run_set_minter_cap_call(input_bytes, caller, value, context)?,
        )),
        WrappedA0GIBase::getWA0GICall::SELECTOR => {
            Ok(stateful_output(gas_used, run_get_wa0gi_call(input_bytes, caller, value, context)?))
        }
        WrappedA0GIBase::minterSupplyCall::SELECTOR => Ok(stateful_output(
            gas_used,
...
fn get_minter_supply<CTX: ContextTr>(
    ...
    if buf.is_empty() {
        return Ok(Supply { cap: U256::ZERO, initialSupply: U256::ZERO, supply: U256::ZERO });
    }
...
fn run_burn_call<CTX: ContextTr>(
    ...
    if supply.supply < args.amount {
        return Err(PrecompileError::Other("insufficient mint supply".to_string()));
    }
    ...
    .is_some()
    {
        return Err(PrecompileError::Other("WA0GI balance insufficient".to_string()));
    }
    set_minter_supply(context, args.minter, supply)?;
...
fn run_set_minter_cap_call<CTX: ContextTr>(
    ...
    match supply.initialSupply.cmp(&args.initialSupply) {
        std::cmp::Ordering::Greater => {
            // old > new -> sub(diff)
            supply.supply = supply.supply.saturating_sub(supply.initialSupply - args.initialSupply);
        }
        std::cmp::Ordering::Less => {
            // old < new -> add(diff)
            supply.supply = supply.supply.saturating_add(args.initialSupply - supply.initialSupply);
        }
    ...
    Ok(Bytes::from(setMinterCapCall::abi_encode_returns(&setMinterCapReturn {})))
}
...
fn run_get_wa0gi_call<CTX: ContextTr>(
    ...
    let _ = getWA0GICall::abi_decode_validate(input_bytes)
        .map_err(|_e| PrecompileError::StatefulInvalidInput)?;
    Ok(Bytes::from(getWA0GICall::abi_encode_returns(&WA0GI_ADDRESS)))
}
